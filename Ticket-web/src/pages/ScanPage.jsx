  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-6 flex items-center gap-2.5">
        ...
        <h1 className="text-xl font-semibold text-neutral-900">Scan ticket</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mb-6 rounded-xl border border-neutral-200 bg-white p-6">
        <label className="mb-2 block text-sm text-neutral-700">
          พร้อมสแกน — เล็งเครื่องสแกนไปที่ QR บนตั๋ว
        </label>
        <input
          ref={inputRef}
          value={input}
        ...
        <p className="mt-2 text-xs text-neutral-400">
          ช่องนี้ต้องอยู่ในโฟกัสตลอดเวลา ถ้าคลิกไปที่อื่นแล้วสแกนไม่ทำงาน ให้คลิกกลับมาที่หน้านี้อีกครั้ง
        </p>
      </form>
        ...
        <div className={`mb-6 rounded-xl border p-6 ${style.bg} ${style.border}`}>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 font-mono text-sm text-neutral-500">โค้ด: {lastResult.scannedCode}</p>
              {lastResult.ticket?.owner && (
                <p className="text-sm text-neutral-500">เจ้าของตั๋ว: {lastResult.ticket.owner}</p>
              )}
            </div>
          </div>
        </div>
      ...
      {history.length > 0 && (
        <div>
          <h2 className="mb-3 text-sm font-medium text-neutral-700">ประวัติการสแกนล่าสุด</h2>
