...
export default function SellTicketPage() {
  ...
  async function handleSell(e) {
    ...
    if (!tickid || !owner.trim()) {
      setError("กรุณาเลือกอีเวนต์และกรอกข้อมูลผู้ซื้อ (เช่น เบอร์โทร)");
      return;
    }
    ...
    try {
      ...
      setError(err.message || "ขายตั๋วไม่สำเร็จ");
    } finally {
      setSelling(false);
    }
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-2">
        <div className="max-h-[75vh] overflow-y-auto rounded-xl border border-neutral-200 bg-white p-6">
          ...
          {loading ? (
            <p className="text-sm text-neutral-400">กำลังโหลดรายการอีเวนต์...</p>
          ) : events.length === 0 ? (
            <p className="text-sm text-neutral-400">ไม่มีอีเวนต์ที่เปิดขายอยู่ในขณะนี้</p>
          ) : (
            <form onSubmit={handleSell} className="space-y-4">
                ...
                >
                  {events.map((e) => (
                    <option key={e.id} value={e.id}>
                      {e.name} — เหลือ {e.stock} ใบ
                    </option>
                  ))}
                </select>
                {selectedEvent && (
                  <p className="mt-1.5 text-xs text-neutral-500">
                    ราคา {currency(selectedEvent.price)} · เหลือ {selectedEvent.stock} ใบ
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="mb-1.5 block text-sm text-neutral-700">
                  Transaction ID (ไม่บังคับ ถ้าเว้นว่างระบบจะสร้างให้อัตโนมัติ)
                </label>
                <input
                  value={tranid}
        ...
        <div className="max-h-[75vh] overflow-y-auto rounded-xl border border-neutral-200 bg-white p-6">
          <h2 className="mb-4 text-base font-medium text-neutral-900">ผลการขายล่าสุด</h2>

          {!sold ? (
            <p className="text-sm text-neutral-400">
              ยังไม่มีการขาย — กรอกฟอร์มด้านซ้ายแล้วกด "ยืนยันการขาย"
            </p>
          ) : (
            <div className="text-center">
              ...
              >
                ...
                ดาวน์โหลด QR ให้ลูกค้า
              </a>
              <p className="mt-4 text-xs text-neutral-400">
                ให้ลูกค้าเก็บโค้ดหรือรูป QR นี้ไว้ แล้วนำมาสแกน/กรอกที่หน้า "Scan ticket" ตอนมารับตั๋วจริง
              </p>
            </div>
          )}
        </div>
